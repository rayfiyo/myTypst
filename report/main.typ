...
= 序論
Typstはmarkdown likeなコーディングでpdf, ポスター,
スライド等のドキュメントを作成できます. Rust言語で書かれており, コンパイルが#LATEX に比べて早いのが特長です.
...
#LATEX はカスタム性の高さ, 歴史的なところからまだまだ廃れないとは思いますが,
卒論や修論や学会の予稿等の作成においてはTypst @madje2022programmable
の使いやすさから置き換わるのではないかと思います(半分願望).
#img(
  image("images/ladder.svg", width: 20%), caption: [The Go gopher was designed by Renée French.],
) <img2>

=== #LATEX はコンパイルが遅い

本資料は, #LATEX でコンパイルの待ち時間中に作りました. 他にも
```typ
#include path.typ
```

とすれば, 他ファイルを参照できるので, 長い分量の本などを作成する際に,
章ごとにファイルを分けるなどができるようになります.\
便利なので広まれば良いなと思います.\
...
カッコ内の引数に人名などを入れることができます. また, identifierを変えれば,
カウントはリセットされる.
identifier毎にカウントを柔軟に変えられるようにしてあるので,
様々な論文の形式に対応できるはずです.
```typ
